...
function orderedCoordsForPipe(ft) {
  // GeoJSON coordinate order is treated as authoritative for flow direction.
  // Do NOT use derived DIR fields (u_to_d / d_to_u) for routing.
...
function buildPipePlanFromObjectId(startObjectId, startLngLat) {
  const terminalIds = new Set([311851, 311905]);
  const maxHops = 4000;
  const startId = Number(startObjectId);

  if (!Number.isFinite(startId) || !pipeById.has(startId)) return null;

  function orderCandidatesByBearing(currId, candidateIds) {
    const currFt = pipeById.get(currId);
    const currCoords = orderedCoordsForPipe(currFt);
    if (!currCoords || currCoords.length < 2) return candidateIds.slice();

    const currA = currCoords[currCoords.length - 2];
    const currB = currCoords[currCoords.length - 1];
    const currBrg = bearingDeg(currA, currB);

    return candidateIds
      .slice()
      .map((cid) => {
        const cft = pipeById.get(cid);
        const ccoords = orderedCoordsForPipe(cft);
        if (!ccoords || ccoords.length < 2) return { cid, score: 1e9 };
        const cA = ccoords[0];
        const cB = ccoords[1];
        const cBrg = bearingDeg(cA, cB);
        const diff = Math.abs((((cBrg - currBrg) + 540) % 360) - 180); // 0..180
        return { cid, score: diff };
      })
      .sort((x, y) => x.score - y.score)
      .map((x) => x.cid);
  }

  function findRouteIds() {
    const inPath = new Set();
    const stack = [];

    function pushNode(id) {
      inPath.add(id);
      stack.push({ id, candidates: null, idx: 0 });
    }

    pushNode(startId);

    while (stack.length && stack.length <= maxHops) {
      const top = stack[stack.length - 1];
      const currId = top.id;

      if (terminalIds.has(currId)) {
        return stack.map((s) => s.id);
      }

      if (!top.candidates) {
        const currFt = pipeById.get(currId);
        const raw = (currFt?.properties?._nextObjectIds || []).map(Number).filter((n) => Number.isFinite(n));
        const uniq = Array.from(new Set(raw)).filter((n) => n !== currId);
        top.candidates = orderCandidatesByBearing(currId, uniq);
        top.idx = 0;
      }

      // Exhausted candidates -> backtrack
      if (top.idx >= top.candidates.length) {
        inPath.delete(currId);
        stack.pop();
        continue;
      }

      const nextId = top.candidates[top.idx++];
      if (inPath.has(nextId)) continue;
      if (!pipeById.has(nextId)) continue;
  ...
  let last = null;
  const EPS_M = 0.02; // de-dup points

  routeIds.forEach((oid, idx) => {
    const ft = pipeById.get(oid);
    if (!ft) return;
    ...
    let startIndex = 0;
    if (idx === 0 && startLngLat && Array.isArray(startLngLat) && startLngLat.length === 2) {
      // Choose the nearest vertex as the start point reference; then travel to the end.
      let bestI = 0;
      let bestD = Infinity;
      for (let i = 0; i < coords.length; i++) {
        const d = haversineM(startLngLat, coords[i]);
        if (d < bestD) {
          bestD = d;
          bestI = i;
        }
      ...
      // Ensure the flush starts exactly where the user clicked.
      pathLngLats.push(startLngLat);
      last = startLngLat;
    }

    for (let i = startIndex; i < coords.length; i++) {
      const pt = coords[i];
      if (!last || haversineM(last, pt) > EPS_M) {
        pathLngLats.push(pt);
        last = pt;
      }
    }
      ...
      try {
        ...
        for (const ft of features) {
          ...
          if (objectId !== null && geomStart && geomEnd) {
            ...
            // Routing must not depend on derived DIR. GeoJSON coordinate order is authoritative.
