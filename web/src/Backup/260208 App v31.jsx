...
function parseObjectId(propsOrValue) {
  // Accept either a properties object or a raw OBJECTID value
  const v = (propsOrValue && typeof propsOrValue === "object") ? (propsOrValue.OBJECTID ?? propsOrValue.ObjectId ?? propsOrValue.objectid) : propsOrValue;
...
function chooseNextPipeWithLookahead(candidateIds, byObjectId, currProps, currId, prevId, visited) {
  ...
  // If we have a current downstream level, try to avoid any candidate that goes "uphill"
...
function orderedCoordsForPipe(ft) {
  // New v20+ flow rule: geometry order defines direction (first -> last)
...
function buildPipePlanFromObjectId(startObjectId, startPoint, byObjectId, maxHops = 6000) {
  // New v20+ flow rule:
  // - Geometry order defines flow direction (first coord -> last coord)
  // - Next pipes are derived purely by exact coordinate-key match: endKey -> startKey
  // - At forks, do DFS with backtracking until we reach a target sink pipe.
  const TARGET_IDS = new Set([311851, 311905]);

  let currentId = startObjectId;
  let prevId = null;

  // Track traversal and allow backtracking
  let pathIds = currentId !== null ? [currentId] : [];
  let visited = new Set(pathIds);
  const forkStack = []; // { atId, remaining: number[] }

  // Build polyline points (downsampled on a 5m grid like v17)
  const plan = [];
  const visitedCells = new Set();

  let steps = 0;

  while (currentId !== null && steps < maxHops) {
    const ft = byObjectId.get(currentId);
    if (!ft) break;

    // Add this pipe's geometry to the plan (geometry order)
    const ord = flattenFeatureCoords(ft);
    if (ord.length >= 2) {
    ...
    const p = ft.properties || {};
    const nextAll = Array.isArray(p._nextObjectIds) ? p._nextObjectIds : [];

    // Success: reached a target sink pipe
    if (TARGET_IDS.has(currentId)) {
      if (visited.size < 200) {
        console.log(
          "[PIPEHOP] " +
            currentId +
    ...
    // Candidate next pipes are those that start exactly where this pipe ends
    const candidates = nextAll.filter((id) => !visited.has(id));

    if (candidates.length === 0) {
      // Dead-end in this directed-by-geometry graph: backtrack to last fork
      let found = null;

      while (forkStack.length > 0 && !found) {
        const top = forkStack[forkStack.length - 1];
        const nxt = top.remaining.shift();
        if (nxt !== undefined && nxt !== null) {
          found = { forkId: top.atId, nextId: nxt };
        } else {
          forkStack.pop();
        }
      }
      ...
      // Truncate path to forkId and resume with alternative candidate
      const idx = pathIds.lastIndexOf(found.forkId);
      if (idx === -1) {
        currentId = null;
        break;
      }
      ...
      prevId = idx > 0 ? pathIds[idx - 1] : null;
      currentId = found.nextId;
    ...
    // Choose best next candidate by "straightest continuation" bearing
    const nextId = chooseNextPipeByBearing(candidates, byObjectId, ord, prevId, visited);

    // Save remaining candidates for backtracking at this fork
    const remaining = candidates.filter((id) => id !== nextId);
    if (remaining.length > 0) forkStack.push({ atId: currentId, remaining });
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // v20+ connectivity: geometry order only (first coord -> last coord)
        // Build an exact coordinate-key index of pipe starts, then derive next pipes by endKey -> startKey.
        const byObjectId = new Map();
        const startIndex = new Map(); // startKey -> number[]
        const KEY_DECIMALS = 6;

        function coordKeyFromLL(ll) {
          const lng = Number(ll?.lng);
          const lat = Number(ll?.lat);
          if (!isFinite(lng) || !isFinite(lat)) return null;
          return lng.toFixed(KEY_DECIMALS) + "," + lat.toFixed(KEY_DECIMALS);
        }

        function pushIndex(map, key, objectId) {
          if (!key || objectId === null) return;
          const arr = map.get(key);
          if (arr) arr.push(objectId);
          else map.set(key, [objectId]);
        }

        // 1) index all pipes by OBJECTID and record their start/end keys (geometry order)
        for (const ft of features) {
          const props = ft.properties || {};
          const objectId = parseObjectId(props.OBJECTID ?? props.ObjectId ?? props.objectid);
          ...
          const startKey = coordKeyFromLL(startLL);
          const endKey = coordKeyFromLL(endLL);

          props._startKey = startKey;
          props._endKey = endKey;

          // keep normalised name for debug / potential future heuristics
          props._sewer_name_norm = normaliseSewerName(props.SEWER_NAME || props.SEWERNAME);

          pushIndex(startIndex, startKey, objectId);
        }

        // 2) derive next candidates for each pipe by exact key match: endKey -> startKey
        for (const [objectId, ft] of byObjectId.entries()) {
          const p = ft?.properties || {};
          const endKey = p._endKey;
          const next = startIndex.get(endKey) || [];
          p._nextObjectIds = next.filter((id) => id !== objectId);
        }
