...
function isValidLatLngObj(o) {
  return o && isFiniteNum(o.lat) && isFiniteNum(o.lng);
}
function toLatLng(o) {
  ...
  for (const ft of features) {
    ...
    const considerLineString = (coords, linePartIndex) => {
      for (let i = 0; i < coords.length - 1; i++) {
        ...
        const a = { lng: aLng, lat: aLat };
        const b = { lng: bLng, lat: bLat };
        const res = closestPointOnSegment(a, b, start);
        const rp = res?.point;
        if (!rp || !Number.isFinite(rp.lat) || !Number.isFinite(rp.lng) || !Number.isFinite(res.dist)) continue;
...
function buildPipePlanFromObjectId(startObjectId, startPoint, byObjectId, maxSteps = 5000) {
...
for (const ft of features) {
  ...
  async function buildStreetRouteForPoint(pointId, startLL) {
    ...
    // Build the pipe plan regardless of whether OSRM (street routing) is available.
    const pipePlan =
      pipeData.byObjectId && objectId !== null
        ? buildPipePlanFromObjectId(objectId, contact.point, pipeData.byObjectId)
        : null;

    // Try OSRM; if it fails, still keep contact + pipePlan so the green route renders.
    try {
      ...
      console.warn("OSRM street route failed; showing pipe path only.", e);
      ...
      setPoints((prev) =>
        prev.map((pt) => {
          ...
          if (pt.mode === "street" && pt.street && Array.isArray(pt.street.route)) {
            ...
            const targetRaw = route[idx];
            const target = isValidLatLngObj(targetRaw) ? toLatLng(targetRaw) : null;

            if (!target) {
              return { ...pt, mode: "error", error: "Street route contained invalid coordinate" };
            }

            const here = { lat: Number(pt.lat), lng: Number(pt.lng) };
          ...
          if (pt.mode === "pipe" && pt.pipePlan && Array.isArray(pt.pipePlan)) {
            ...
            const targetRaw = plan[idx];
            const target = isValidLatLngObj(targetRaw) ? toLatLng(targetRaw) : null;

            if (!target) {
              return { ...pt, mode: "error", error: "Pipe plan contained invalid coordinate" };
            }
    ...
    <div className="layout">
      ...
      <div className="map">
        ...
        >
          ...
          {points.filter((p) => Number.isFinite(Number(p.lat)) && Number.isFinite(Number(p.lng))).map((p) => (
            <CircleMarker
              key={p.id}
