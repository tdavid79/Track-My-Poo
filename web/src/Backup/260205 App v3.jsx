    ...
    async function load() {
      try {
        ...
        for (const ft of features) {
		  ...
		  // --- Virtual connector: if no outgoing pipe, jump to nearest endpoint within 5 m ---
		  if (next.length === 0) {
			const nearKey = findNearestNodeKeyWithinMeters(nodeIndex, downKey, 5);
			if (nearKey) {
			  const nearNode = nodeIndex.get(nearKey);
			  const nearNext = Array.isArray(nearNode?.outObjectIds) ? nearNode.outObjectIds : [];
			  next = nearNext.filter((id) => id !== objectId);
