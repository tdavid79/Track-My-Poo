  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Pass 2: compute next connections for each pipe (STRICT DIR-ONLY CONNECTIVITY)
        // Next pipes are ONLY those whose upstream node is exactly this pipe's downstream node,
        // as pre-indexed in nodeIndex. No proximity snaps. No virtual jumps. No name/IL heuristics.
        for (const ft of features) {
          const props = ft?.properties || {};
          const objectId = toNum(props.OBJECTID);
