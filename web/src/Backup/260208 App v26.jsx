          ...
          {showStreetRoutes &&
            points
              ...
              .map((p) => (
                <Polyline
                  key={`street-${p.id}`}
                  positions={p.street.route.filter((pt) => pt && Number.isFinite(Number(pt.lat)) && Number.isFinite(Number(pt.lng))).map((pt) => [Number(pt.lat), Number(pt.lng)])}
                  pathOptions={{
                    color: "#8b5a2b",
                    weight: 4,
                    opacity: 0.8
          ...
          {showPipePlans &&
            points
              ...
              .map((p) => (
                <Polyline
                  key={`pipeplan-${p.id}`}
                  positions={p.pipePlan.filter((pt) => pt && Number.isFinite(Number(pt.lat)) && Number.isFinite(Number(pt.lng))).map((pt) => [Number(pt.lat), Number(pt.lng)])}
                  pathOptions={{
                    color: "#00aa00",
                    weight: 5,
                    opacity: 0.85
