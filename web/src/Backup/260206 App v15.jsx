...
function buildPipePlanFromObjectId(startObjectId, startPoint, byObjectId, nodeIndex, upstreamIndex, maxHops = 2000) {
  // DIR-only traversal:
  ...
  for (let hop = 0; hop < maxHops; hop++) {
    ...
    // Fallback: if this pipe ends at a node with no outgoing connections (common at pump stations / pits),
    // join to the nearest OTHER pipe's upstream endpoint within a radius. Still respects DIR:
    // we only consider OTHER pipes' *upstream* endpoints as join targets.
    let candidateIds = nextIds;
    let usedFallback = false;

    if (candidateIds.length === 0 && nodeIndex && upstreamIndex && props._downNodeKey) {
      const dn = nodeIndex.get(props._downNodeKey);
      const downPt = dn ? { lat: dn.lat, lng: dn.lng } : null;
      const currName = dbgName(props);

      if (downPt) {
        const scored = [];
        for (let i = 0; i < upstreamIndex.length; i++) {
          const rec = upstreamIndex[i];
          if (!rec) continue;
          if (rec.id == null || rec.id === currentId) continue;
          if (visited.has(rec.id)) continue;
          if (prevId != null && rec.id === prevId) continue;

          // Keep it conservative: only join within the same named sewer (if name exists)
          if (currName && rec.name && rec.name !== currName) continue;

          const d = metersBetween(downPt, rec.up);
          if (d <= FALLBACK_CONNECT_M) scored.push([d, rec.id]);
        }

        scored.sort((a, b) => a[0] - b[0]);
        candidateIds = scored.slice(0, 25).map((x) => x[1]);
        if (candidateIds.length > 0) usedFallback = true;
      }
    }

    const nextId = chooseNextPipeDirOnly(candidateIds, byObjectId, ord, prevId, visited);

    // Debug
    if (visited.size < 80) {
      const currName = dbgName(props);
      const cand = candidateIds.join(", ");
      console.log(
        "[PIPEHOP] " +
          currentId +
          " -> " +
...
for (const ft of features) {
  ...
  if (objectId !== null && start && end) {
    ...
    // DIR defines which geometry end is upstream vs downstream
...
// Build quick lookup of each pipe's upstream endpoint coordinate (cluster centroid)
// Used only as a conservative fallback join when a node has no outgoing pipes.
const upstreamIndex = [];
for (const ft of features) {
  const p = ft?.properties || {};
  const id = toNum(p.OBJECTID);
  const upK = p._upNodeKey;
  if (id === null || !upK) continue;
  const upNode = nodeIndex.get(upK);
  if (!upNode) continue;
  upstreamIndex.push({
    id,
    name: dbgName(p),
    up: { lat: upNode.lat, lng: upNode.lng }
  });
}
    ...
    try {
	  ...
	  const pipePlan =
	    pipeData.byObjectId && objectId !== null
		  ? buildPipePlanFromObjectId(objectId, contact.point, pipeData.byObjectId, pipeData.nodeIndex, pipeData.upstreamIndex)
		  : null;
  ...
  function enterPipeMode(pointId) {
    setPoints((prev) =>
      prev.map((pt) => {
        ...
        const plan = buildPipePlanFromObjectId(objectId, contactPoint, pipeData.byObjectId, pipeData.nodeIndex, pipeData.upstreamIndex);
