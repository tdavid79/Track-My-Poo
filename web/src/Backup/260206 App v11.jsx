...
function lookaheadScore(startId, byObjectId, currProps, currId, prevId, visited, depth) {
  ...
  while (id != null && hops < depth) {
    ...
    // Use the same candidate-selection logic during lookahead so DIR-driven segments
    // don't get incorrectly forced by IL-based heuristics.
    const stepId = stepIds.length > 0 ? chooseNextPipeWithLookahead(stepIds, byObjectId, p, id, prev, visited) : null;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // --- Sewer-name connector (DISABLED when DIR is present) ---
      // When DIR is present, the dataset already describes the intended flow direction.
      // Name-based jumps can introduce false shortcuts and cause visible "bouncing".
      if (next.length === 0 && String(props._dir_source || "") !== "DIR") {
        const downNodeLL = nodeIndex.get(downKey);
      ...
      // --- Segment connector (DISABLED when DIR is present) ---
      // Segment-to-point snapping is a last-resort and can jump across nearby parallel pipes.
      // If DIR is present, rely on endpoint/junction connectivity instead.
      if (next.length === 0 && String(props._dir_source || "") !== "DIR") {
        const downNodeLL = nodeIndex.get(downKey);
