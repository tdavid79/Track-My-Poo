...
function chooseNextPipeLowestDownIL(nextIds, byObjectId, currentProps) {
  ...
  const currentName = String(currentProps?.SEWER_NAME || currentProps?.SEWERNAME || "").trim();
...
function buildPipePlanFromObjectId(startObjectId, startPoint, byObjectId, maxHops = 2000) {
  ...
  while (currentId !== null && !visited.has(currentId) && visited.size < maxHops) {
    ...
    const nextId = chooseNextPipeLowestDownIL(nextIds, byObjectId, p);
...
function findNearestPipeObjectIdBySewerNameWithinMeters(geojson, pointLL, sewerNameRaw, excludeObjectId, maxMeters) {
  const targetName = String(sewerNameRaw || "").trim();
  ...
  for (const ft of features) {
    const props = ft?.properties || {};
    const nm = String(props.SEWER_NAME || props.SEWERNAME || "").trim();
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // --- Pump-station connector: if still no outgoing pipe, jump to nearest pipe with same SEWER_NAME ---
      if (next.length === 0) {
        const downNodeLL = nodeIndex.get(downKey);
