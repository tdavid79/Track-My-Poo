    ...
    async function load() {
      try {
        ...
        for (const ft of features) {
          ...
          // Direction: prefer explicit DIR; fall back to IL; else default u_to_d
        ...
        // Pass 2: directed next connections strictly by node-match; if none, do a small
        // endpoint-to-endpoint fallback to the nearest FLOW-START endpoint within radius.
        const FALLBACK_CONNECT_M = 80;
        ...
        for (const ft of features) {
          ...
          // Fallback: if no outgoing candidate, connect to nearest FLOW-START endpoint
          // of a pipe with the same SEWER_NAME within FALLBACK_CONNECT_M.
          if (next.length === 0 && downNode) {
            const fromLL = { lat: downNode.lat, lng: downNode.lng };
            const targetName = props._sewer_name_norm || "";
            let bestId = null;
            let bestDist = Infinity;

            for (const [id, candFt] of byObjectId.entries()) {
              if (id === objectId) continue;
              const cp = candFt?.properties || {};
              if (targetName && cp._sewer_name_norm && cp._sewer_name_norm !== targetName) continue;

              const upKey = cp._upNodeKey;
              const upNode = upKey ? nodeIndex.get(upKey) : null;
              if (!upNode) continue;

              const d = metersBetween(fromLL, { lat: upNode.lat, lng: upNode.lng });
              if (d <= FALLBACK_CONNECT_M && d < bestDist) {
                bestDist = d;
                bestId = id;
              }
            }
            ...
            if (bestId !== null) {
              next = [bestId];
              props._fallback_next_objectid = bestId;
              props._fallback_next_dist_m = bestDist;
            }
          }
